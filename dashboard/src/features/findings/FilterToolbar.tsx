...
export function FilterToolbar({ filters, onChange, resultCount, modules, scans, programs }: FilterToolbarProps) {
  ...
  const FULL_MODULES = new Set(modules);
  const FULL_SCANS = new Set(scans);
  const FULL_PROGRAMS = new Set(programs);
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", padding: "10px 0" }}>
      ...
      <FilterChip label="Module" allValues={modules} selected={filters.modules.size > 0 ? filters.modules : FULL_MODULES} fullSet={FULL_MODULES} onSelect={(s) => update({ modules: s.size === FULL_MODULES.size ? new Set() : s })} />
      <FilterChip label="Scan" allValues={scans} selected={filters.scans.size > 0 ? filters.scans : FULL_SCANS} fullSet={FULL_SCANS} onSelect={(s) => update({ scans: s.size === FULL_SCANS.size ? new Set() : s })} />
      <FilterChip label="Program" allValues={programs} selected={filters.programs.size > 0 ? filters.programs : FULL_PROGRAMS} fullSet={FULL_PROGRAMS} onSelect={(s) => update({ programs: s.size === FULL_PROGRAMS.size ? new Set() : s })} />
